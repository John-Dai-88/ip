...
|         To do task          |                         `todo <TASK_DESCRIPTION>`                          |                        `todo Buy a book`                         |                        Creates a todo task                        |
|  Deadline task (Date only)  |               `deadline <TASK DESCRIPTION> /by <yyyy-mm-dd>`               |              `deadline Return book /by 2026-07-20`               |                 Creates a date only deadline task                 |
...
|      Listing of tasks       |                                    `list`                                    |                             `list`                              |                       List all stored tasks                       |
|        Marking task         |                          `mark <VALID_TASK_NUMBER>`                          |                            `mark 3`                            |                    Marks a task status as done                    |
|       Unmarking task      |                         `unmark <VALID_TASK_NUMBER>`                         |                           `unmark 5`                           |                 Marks a task's status as undone               |
| Deletion of task |                         `delete <VALID_TASK_NUMBER>`                         |                           `delete 8`                           | Deletes a task from Jarvis's list of stored tasks |
| Finding tasks |                `find <KEY_WORD>` (Of more than 2 characters)                 |                          `find book`                           | Finds all tasks that fully/partially matches the key word |
|Exiting the program |                                    `bye`                                     |                             `bye`                              | Quit and exits Jarvis |
...
- **For mark, unmark and delete command** : The `<VALID_TASK_NUMBER>` parameter is referring to the task index numbers from the output of `list` (Refer below for examples)
  ...
  ![Sample photo of task index numbers](https://john-dai-88.github.io/ip/ValidTaskNumberReference.png)
